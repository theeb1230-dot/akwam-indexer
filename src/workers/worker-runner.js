...
async function runClaimedJob(job, options) {
  ...
  const timer = setInterval(() => {
    Promise.resolve(store.heartbeat(job.id, options.workerId, leaseMs)).catch(error => {
      logger.error("job_heartbeat_failed", { job_id: job.id, error_code: error?.code || "HEARTBEAT_FAILED" });
    });
  }, heartbeatMs);
...
async function runWorker(options) {
  ...
  logger.info("worker_started", { role: options.role, worker_id: id });
  ...
  logger.info("worker_stopped", { role: options.role, worker_id: id });
}
