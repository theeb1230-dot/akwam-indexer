...
- توحيد أخطاء Providers والمهلات وإلغاء الطلبات: safe-media-request يفرض timeout bounded افتراضيًا ويحوّل timeout/cancellation إلى REQUEST_TIMEOUT وREQUEST_CANCELLED مع دعم AbortSignal.
...
- التسجيل المنظم ومراقبة الصحة: HTTP errors وserver startup وworkers/schedulers تمر عبر logger منقح، والمقاييس المحلية تدعم تقييم alerts محمية للـ5xx/latency/open circuits بدون خدمة مدفوعة. عتبات warning/critical أصبحت fail-closed بترتيب إلزامي، والتقرير يوضح إن كان الحد الأدنى من عينات HTTP تحقق؛ readiness probes الناجحة تُخزن لفترة قصيرة bounded مع coalescing للطلبات المتزامنة لتقليل ضغط قاعدة البيانات، بينما الفشل لا يُخزن. الضبط النهائي للقيم يبقى معتمدًا على بيانات التشغيل الفعلية.
