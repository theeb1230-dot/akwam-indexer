...
- فصل إنشاء التطبيق عن `listen()`: مثبت بعقد اختبار؛ `server.js` يصدّر Express app مستقلًا ويمكن استضافته عبر HTTP server خارجي، بينما `startServer()` هو حد فتح socket الصريح.
